...
* First panel: Mean-CV plot, CV and mean of batch-corrected data including all cells across all three indivivduals.

* Second panel: Venn diagram of gene-specific mean and adjusted CV for when we only include cells in which the gene was detected as expressed.

* Third panel: Venn diagram of gene-specific mean and adjusted CV for when all cells are included. For more results for when including all cells, see [here][results-all-cells]. 

[results-all-cells]: http://jdblischak.github.io/singleCellSeq/analysis/cv-adjusted-summary-pois.html


```{r figure05, fig.width = 8, fig.height=4}
#### first panel <- mean-cv relationship
tiff(file = "figure/cv-adjusted-summary-pois-final.Rmd/Fig05-01.tiff", 
     res = 300, units = "in", height = 3.7, width = 4,
     compression = "none")
...
tiff(file = "figure/cv-adjusted-summary-pois-final.Rmd/Fig05-02.tiff", 
     res = 300, units = "in", height = 3.7, width = 4,
     compression = "zip")
...
points(y = expressed_cv_subset$all$expr_cv,
       ...
       col = rev(RColorBrewer::brewer.pal(10, "RdYlBu"))[
                cut(drop_out$all, breaks = seq(0, 1, by = .1),
                     include.lowest = TRUE)], 
       cex = .6, pch = 16)
# add labels to the plot on the right
points(x = rep(14.7,10), y = seq(1,4,length.out = 10), 
     pch = 15, cex = .7, 
     col = (RColorBrewer::brewer.pal(10, "RdYlBu")))
text(labels = rev( levels(cut(drop_out$NA19098, breaks = seq(0, 1, by = .1),
                     include.lowest = TRUE))) ,
     y = seq(1,4,length.out = 10), x = 15, cex = .8, adj = 0)
text(labels = "drop-out rate", x = 15.7, y = 4.3, cex = .8)
dev.off()



#### second panel <---- venn diagram for expressed cells analysis

tiff(file = "figure/cv-adjusted-summary-pois-final.Rmd/Fig05-03.tiff", 
     res = 300, units = "in", height = 3.7, width = 4,
     compression = "zip")
...
venn_mean_rank <- gplots::venn( 
  list(NA19098 = genes[ which(rank(expressed_cv_subset[[1]]$expr_mean) > length(genes) - 1000 ) ],
       ...
       NA19239 = genes[ which(rank(expressed_cv_subset[[3]]$expr_mean) > length(genes) - 1000 ) ] ), show.plot = TRUE)
dev.off()

tiff(file = "figure/cv-adjusted-summary-pois-final.Rmd/Fig05-04.tiff", 
     res = 300, units = "in", height = 3.7, width = 4,
     compression = "zip")
# adjusted CV
venn_cv_rank <- gplots::venn( 
  list(NA19098 = genes[ which(rank(expressed_dm_subset[[1]]) > length(genes) - 1000 ) ],
       ...
       NA19239 = genes[ which(rank(expressed_dm_subset[[3]]) > length(genes) - 1000 ) ] ), show.plot = TRUE)
dev.off()


#### thirs panel <---- venn diagram for all cells analysis
tiff(file = "figure/cv-adjusted-summary-pois-final.Rmd/Fig05-05.tiff", 
     res = 300, units = "in", height = 3.7, width = 4,
     compression = "zip")
load("../data/cv-all-cells.rda")
...
tiff(file = "figure/cv-adjusted-summary-pois-final.Rmd/Fig05-06.tiff", 
     res = 300, units = "in", height = 3.7, width = 4,
     compression = "zip")
gplots::venn( 
...
tiff("figure/cv-adjusted-summary-pois-final.Rmd/pluri-dens.tiff",
    height = 12, unit = "in", width = 10, res = 300,
    compression = "zip")
par(mfrow = c(5,4), mar=c(5,4,4,2))
...
## Suppmental figure 10: 

* Top panel: CV versus mean before mean correction. Coefficients of variation plotted against average molecule counts across cells of each individual cell line. Grey dots represent endogeneous genes, and blue dots indicate ERCC spike-in control genes. Red curve depicts the expected coefficients of variation assuming the endogeneous genes follow a poisson distribution. Likewise, blue curve depicts the expected CVs of the ERCC spike-in control genes. Yellow curve predicts the expected CVs assuming standard deviation is 3 times the ERCC spike-in genes. 

* Bottom panel: Adjusted CV versus mean. Investigating distance-to-the-median (DM). (A) to (C) correspond to cell lines NA19098, NA19101, NA19239. DM values of each gene are plotted against log10 of the average molecule counts. 

```{r, fig.width=8, fig.height=6}
tiff(file = "figure/cv-adjusted-summary-pois-final.Rmd/FigS10.tiff", 
     res = 300, units = "in", height = 8, width = 11,
     compression = "zip")
theme_set(theme_bw(base_size = 12))
...
tiff(file = "figure/cv-adjusted-summary-pois-final.Rmd/FigS5.tiff", 
     res = 300, units = "in", height = 9, width = 9, 
     compression = "zip")
plot_grid(
...
#### <---- expressed cells
tiff(file = "figure/cv-adjusted-summary-pois-final.Rmd/FigS4-01.tiff", 
     res = 300, units = "in", height = 3, width = 8,
     compression = "zip")
par(mfrow = c(1,3))
# abundance
...
#### <---- all cells
tiff(file = "figure/cv-adjusted-summary-pois-final.Rmd/FigS4-02.tiff", 
     res = 300, units = "in", height = 3, width = 8,
     compression = "zip")
